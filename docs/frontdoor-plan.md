...
> **Status: M1–M5 shipped** and merged to `main`. Engine untouched, full suite green
> (185 tests), verified via headless render (`sprig-gui render <dir>` → `main_home`, `home_empty`,
> `home_running`, `main_guide`, and the upstream-aware empty states). The milestones below record
> the scope and exit criteria each one shipped against.
