...
public partial class TourGuideViewModel : ViewModelBase
{
    ...
    readonly IReadOnlyList<TourStop> _stops;

    public TourGuideViewModel(Navigator nav)
    {
        _nav = nav;
        _stops = Script();
    }
    ...
    /// <summary>Show the strip and go to the first stop.</summary>
    public void Start()
    {
        Index = 0;
        IsActive = true;
    ...
    /// <summary>Advance to the next stop, or finish the tour's narration on the last one.</summary>
    [RelayCommand]
    private void Next()
    {
    ...
    /// <summary>
    /// The script. Five stops that walk the one-directional model — a repo declares, a stack supplies,
    /// a workspace materialises — and then hand the user back to their own repos.
    /// </summary>
    static IReadOnlyList<TourStop> Script() =>
    [
        new("THE SHAPE OF IT",
