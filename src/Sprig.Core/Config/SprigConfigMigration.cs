...
/// root). <see cref="SprigRepoConfig.Inputs"/> are shared at the repo level and never move.
/// Schema-3+ files are trusted as-is (they were written module-shaped and validated), so migration never
/// re-folds over them. The upgrade is in-memory; it persists the next time the config is saved.
/// Mirrors <see cref="Stacks.StackMigration"/> — forward-tolerant (<c>&gt;=</c>), idempotent.
/// </summary>
